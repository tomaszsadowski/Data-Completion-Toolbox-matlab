function [Y,res]= SCH_NTF_ICH(X,J,ohm,iter,er,X0)
% Segmented Convex Hull NTF for Image Completion, Horizontal Version
% [Y,res]= SCH_NTF_ICH(X,J,ohm,iter,er,X0)
%Inputs:
...
res(q)=norm(tensor(Y-X0))/norm(tensor(X0));
   ...
   while (q<iter)&&(res(end)<er)
        ...
        %Select extreme rays    
        if (q == 2) | ~mod(q,10)
            K_final = micro_model_means_n_way_slices(tensor(Y),J,2);
        end
      ...
      %% updating F by (7) modified
        ...
        F = RuleF_proxi_coordinate(Y,Z,F,q);     
        
        %updating Y by (8) 
        Y_old = Y;
        Yk=double(ttm(tensor(Z),F,1));
        Y(ohm_bar) = Yk(ohm_bar);        
        
        res_ohm(q) = norm(X(ohm) - Yk(ohm))/norm(X(ohm));
        disp(['SCH_NTF_IC_H: iter: ' , num2str(q), ', res_ohm: ' , num2str(res_ohm(q))])
       ...
       % update z
        ...
        Y(ohm) = X(ohm);      
        res(q)=norm(tensor(Y-X0))/norm(tensor(X0));
      %  disp(['SCH_NTF_IC_H: iter: ' , num2str(q), ', res: ' , num2str(res(q))])
        if q>2
